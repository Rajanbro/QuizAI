...
import { saveHighScore } from './app.js';

let questionHistory = JSON.parse(sessionStorage.getItem("questionHistory") || "[]");

async function fetchExplanation(question, correctAnswer, language) {
  try {
    const res = await fetch(`/api/explanation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ question, correctAnswer, language })
    });
...
export async function nextQuestion() {
  ...
  if (currentQ && currentQ.options) {
    ...
    explanationBox.textContent = 'Fetching explanation...';
    const language = document.getElementById('language-selector')?.value || 'en';
    const explanation = await fetchExplanation(currentQ.question, currentQ.correct_answer, language);
